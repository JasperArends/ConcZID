...
gini_gamma.est <- function(X, Y) {
  ...
  # Concordance w.r.t. M and W
  Q.HM <- QM.est(X, Y)
  Q.HW <- QW.est(X, Y)
  
  # Centering
  Q.P <- spm_rho.bound_est(X, Y)
  
  return ( Q.HM + Q.HW - (Q.P$up + Q.P$lw)/3 )
}
...
spm_foot.est <- function(X, Y) {
  ...
  # Centering
  Q.PM <- 1 - max(sum(X==0)/N,sum(Y==0)/N)^3
  
  return ( 0.5 * (3 * Q.HM - Q.PM) )
}
...
spm_rho.est <- function(X, Y) {
  ...
  rho11 <- ifelse(length(D11) > 1, cor(X[D11], Y[D11], method="spearman"), 0)
  ...
  rho01 <- ifelse(length(D01) > 0 & length(D11) > 0,
                  ...
                  0)
  rho00 <- ifelse(length(D11) > 0 & length(D10) > 0 & length(D01) > 0,
                  12 * mean( colMeans(outer(X[D10], X[D11], FUN = ">")) *
...
gini_gamma.bound <- function(p1, p2) {
  ...
  gamma.up <- ifelse(p1 + p2 >= 1,
                     2/3 + 1/3 * max(p1,p2) * (3 - 6 * max(p1,p2) + max(p1,p2)^2),
                     1 - max(p1,p2)^2 - 1/3 * min(p1,p2)^3)
  
  # Lower bound
  gamma.lw <- ifelse(p1 + p2 >= 1,
                     (1 - max(p1,p2)) * (p1 + p2 + min(p1,p2) - 2) - 1/3 * (1 - max(p1,p2)^3),
                     (1 - p1 - p2)^2 - 2 * (1 - p1)*(1 - p2) - 1/3 * min(p1,p2)^3)
...
spm_foot.bound <- function(p1, p2) {
  # Upper bound
  spm_foot.up <- 1 - 3/2 * max(p1,p2)^2 + 1/2 * max(p1,p2)^3
  
  # Lower bound
  spm_foot.lw <- 3/2 * max(0, p1 + p2 - 1) * (1 - max(p1,p2)) - 1/2 * (1 - max(p1,p2)^3)
...
gini_gamma.bound_est <- function(X, Y) {
  ...
  # Upper bound
  gamma.up <- gini_gamma.bound(p1, p2)$up
  
  # Lower bound
  gamma.lw <- gini_gamma.bound(p1, p2)$lw
